...
class ServiceLines:
    ...
    def get(self):
        for i in range(len(self.dataList)):
            ...
            Av_ServiceLineHeader = ServiceLineHeader(self.dilimiter, i+1, self.dataList[i].get_proc_code(), '{:.2f}'.format(v.get_units() * v.get_rate()), v.get_units(), v.get_zip(), diagPointers, v.clock_in+'-'+v.clock_out, self.rangedService, repeatedService).getSegment()
